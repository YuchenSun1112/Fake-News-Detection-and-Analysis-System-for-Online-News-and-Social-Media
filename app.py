...
from src.runtime import cleanup_blat_temp_files, configure_temp_dir

configure_temp_dir()
cleanup_blat_temp_files()
...
from src.topic_analysis_page import render_topic_analysis_page
from src.config import TOP_K_CLAIMS
...
def split_article_into_retrieval_units(article: str, max_sentences: int = TOP_K_CLAIMS):
    sentences = [
        sentence.strip()
        for sentence in re.split(r"(?<=[.!?])\s+", article.strip())
        if sentence.strip()
    ]
    claims = []
...
def render_final_result(container, final_result, is_partial=False):
    with container.container():
        ...
        if is_partial:
            st.caption("Partial result based on the sentences processed so far.")
...
def render_extraction_progress(container, extraction_events, current_claims):
    with container.container():
        st.markdown("## Sentence Search Units")
        if not extraction_events:
            st.info("Waiting for sentence splitting...")
        else:
            for event in extraction_events:
                ...
                with st.expander(f"Chunk {chunk_id}/{total_chunks}", expanded=(chunk_id == 1)):
                    ...
                    if chunk_claims:
                        ...
                        st.caption("Using original sentences directly for retrieval.")

        st.markdown("### Current Search Units")
        if current_claims:
            for claim in current_claims:
                st.write(
                    f"{claim.get('source_sentence_id', '?')}. {claim.get('text', '')}"
                )
        else:
            st.caption("No search units selected yet.")


def render_claim_results(container, claim_results, total_claims):
    with container.container():
        st.markdown("## Sentence Verification Results")
        if not claim_results:
            st.info("Waiting for sentence verification results...")
            return
        ...
        for idx, item in enumerate(claim_results, start=1):
            ...
            st.markdown(f"### Sentence {idx}/{total_claims}")
            st.write(claim_text)
        ...
        if remaining:
            st.caption(f"{remaining} sentence(s) still processing...")
...
with tab_verify:
    st.markdown(
        "Paste a news article below. The system will split it into sentences, retrieve evidence, "
    ...
    if st.button("Verify News"):
        if not user_input.strip():
            ...
            status_placeholder.info("Splitting article into sentence-level search units...")
            extraction_events = [
                {
                    "stage": "sentence_split",
                    "chunk_id": 1,
                    "total_chunks": 1,
                    "chunk_text": cleaned_article,
                    "summary": "Using original sentences directly for GNews retrieval.",
                }
            ]
            claims = split_article_into_retrieval_units(cleaned_article)
            ...
            if not claims:
                ...
                status_placeholder.warning("No sentence-level search units were found.")
            else:
                ...
                for idx, claim in enumerate(claims, start=1):
                    status_placeholder.info(
                        f"Processing sentence {idx}/{total_claims}: retrieving evidence..."
                    )

                    retrieval_text = claim.get("source_sentence") or claim["text"]
                    retrieval_output = retrieve_evidence(retrieval_text)
...
with tab_chat:
    st.markdown(
        "Paste a news article and chat with the system to verify sentence-level evidence, "
            ...
            if chat_input_text.strip():
                ...
                st.session_state.chat_messages = [{
                    ...
                    "content": (
                        ...
                        "- **verify** — fact-check the sentences\n"
                        "- **sentiment** — analyse media tone\n"
                        "- **claims** — list sentence-level search units\n"
    ...
    if user_msg:
        ...
        def _run_pipeline():
            ...
            claims = split_article_into_retrieval_units(cleaned)
            claim_results = []
            for claim in claims:
                retrieval_text = claim.get("source_sentence") or claim["text"]
                retrieval = retrieve_evidence(retrieval_text)
        ...
        if any(kw in q for kw in ["verify", "real", "fake", "true", "false", "check", "fact"]):
            ...
            if r["claims"]:
                top = r["claims"][0]
                ev = top["verification"]
                lines.append(f"\n**Top sentence:** _{top['claim']['text']}_")
            ...
            with st.spinner("Splitting article into search units..."):
                cleaned = clean_article_text(st.session_state.chat_article)
                claims = split_article_into_retrieval_units(cleaned)
            if claims:
                lines = [f"Found **{len(claims)}** sentence-level search unit(s):\n"]
                for i, c in enumerate(claims, 1):
                    lines.append(f"{i}. _{c['text']}_")
                lines.append("\nType **verify** to fact-check these sentences.")
                reply = "\n".join(lines)
            else:
                reply = "No sentence-level search units could be found from this article."
            ...
            reply = (
                "I can help you with:\n"
                "- **verify** — fact-check the article's sentences\n"
