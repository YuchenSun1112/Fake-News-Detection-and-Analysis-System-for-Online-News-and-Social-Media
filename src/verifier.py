...
# ── 推理用的 import（轻量，启动时就加载）────────────────────────────────────────
from transformers import AutoTokenizer, AutoModelForSequenceClassification


# ─────────────────────────────────────────────────────────────────────────────
# 训练相关函数（import 延迟到调用时，避免拖慢 Streamlit 冷启动）
# ─────────────────────────────────────────────────────────────────────────────
...
def train_baseline(debug_sample_size=None):
    ...
    model = BertForSequenceClassification.from_pretrained(BASELINE_MODEL_NAME, config=config)
...
# ─────────────────────────────────────────────────────────────────────────────
# 推理：用 st.cache_resource 缓存，Streamlit 重跑时不重新加载权重
# ─────────────────────────────────────────────────────────────────────────────

def _load_baseline_model_impl():
    """实际加载逻辑，与缓存装饰器分离，方便在非 Streamlit 环境下直接调用。"""
...
try:
    ...
    def load_baseline_model():
        return _load_baseline_model_impl()

except ImportError:
    # 非 Streamlit 环境（脚本、测试）退回 lru_cache
